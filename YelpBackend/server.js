...
//endpoint for a future list of businesses for category
app.get("/businessByCategorie", (req, res) => {
    const object = knex("businesses").select("address", "attributes.Open24Hours").where("categories", req.params.q).then((response) => {
        res.send(response);
    });
    console.log("GET categories request");
})
...
app.get("/search/", (req, res) => {
    ...
    whereILike("name", `%${searchQ}%`).then((response) => {
        ...
        //res.write('' + numberOfResults)	// we have to convert the number to a string in order to be send
